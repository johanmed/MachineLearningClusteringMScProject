...
from sklearn.cluster import OPTICS # import OPTICS class for DBSCAN clustering
...
class Columns2Clustering(ModellingDBSCAN):
    ...
    def perform_dbscan_clustering(self):
        ...
        """
        dbscan_clustering=Pipeline([('preprocessing_hits', preprocessing_hits), ('optics_dbscan', OPTICS(cluster_method='dbscan'))])
    ...
    def visualize_plot(plot_dbscan, dbscan, X_train, size=500):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_DBSCAN_clustering_SVM_prediction_result_by_hits"))
        plt.show()
    ...
    def visualize_plot_annotation(X_valid, y_supervised_pred, type_anno):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_DBSCAN_clustering_SVM__{type_anno}annotation_result_by_hits"))
        plt.show()
...
def main():
    ...
    extracted_annotation=Columns2Clustering.extract_features_target_relationship(X_valid_features, y_train, X_valid_features, y_valid)
