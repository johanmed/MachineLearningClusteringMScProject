...
class Columns2Clustering:
    ...
    def visualize_plot(plot_dbscan, dbscan, X_train, index1, index2, size=500):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_DBSCAN_clustering_RandomForest_prediction_result_by_hits_{index1}_{index2}"))
        plt.show()
    ...
    def visualize_plot_annotation(X_valid, y_supervised_pred, index1, index2, type_anno):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_DBSCAN_clustering_RandomForest_{type_anno}_annotation_result_by_hits_{index1}_{index2}"))
        plt.show()
