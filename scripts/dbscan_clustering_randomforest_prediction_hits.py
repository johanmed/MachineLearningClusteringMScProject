...
class Columns2Clustering(ModellingDBSCAN):
    ...
    def visualize_plot(plot_dbscan, dbscan, X_train, size=500):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_DBSCAN_clustering_RandomForest_prediction_result_by_hits"))
        plt.show()
    ...
    def visualize_plot_annotation(X_valid, y_supervised_pred, type_anno):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_DBSCAN_clustering_RandomForest_{type_anno}_annotation_result_by_hits"))
        plt.show()
...
print(f"Execution time for dbscan_clustering_randomforest_prediction_hits.py is : {time_taken} seconds")
