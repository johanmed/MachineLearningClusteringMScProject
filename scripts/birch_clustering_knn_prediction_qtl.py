...
KNeighborsClassifier is run on clusters extracted by Birch to predict clustering and description or trait category of validation data
...
class Columns2Clustering(ModellingBirch):
    ...
    def visualize_plot(plot_birch, birch_clustering, X_train, size=200):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_BIRCH_clustering_KNN_prediction_result_by_qtl"))
        plt.show()
    ...
    def visualize_plot_annotation(X_valid, y_supervised_pred, type_anno):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_BIRCH_clustering_KNN_{type_anno}_annotation_result_by_qtl"))
        plt.show()
