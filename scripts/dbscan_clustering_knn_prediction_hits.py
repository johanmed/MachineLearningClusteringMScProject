...
class Columns2Clustering(ModellingDBSCAN):
    ...
    def visualize_plot(plot_dbscan, dbscan_clustering, X_train, size=500):
        """
        Generate actual visualization of clusters
        Save figure
        """
        plt.figure(figsize=(10, 10))
        plot_dbscan(dbscan_clustering, X_train, size)
        plt.savefig(os.path.join(out_dir, f"Project_PCA_DBSCAN_clustering_KNN_prediction_result_by_hits"))
        plt.show()
