...
class Columns2Clustering(ModellingKMeans):
    ...
    def visualize_plot(plot_kmeans, clusterer, X_train):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_KMeans_clustering_KNN_prediction_result_by_qtl"))
        plt.show()
    ...
    def visualize_plot_annotation(X_valid, y_supervised_pred, type_anno):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_KMeans_clustering_KNN_{type_anno}_annotation_result_by_qtl"))
        plt.show()
...
print(f"Execution time for kmeans_clustering_knn_prediction_qtl.py is : {time_taken} seconds")
