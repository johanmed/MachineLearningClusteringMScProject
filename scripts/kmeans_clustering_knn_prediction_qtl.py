...
class Columns2Clustering:
    ...
    def visualize_plot(plot_kmeans, clusterer, X_train, index)
        ...
        plt.savefig(os.path.join(out_dir, f"Project_KMeans_clustering_KNN_prediction_result_by_qtl_{index}")))
        plt.show()
    ...
    def visualize_plot_annotation(X_valid, y_supervised_pred, index, type_anno):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_KMeans_clustering_KNN_{type_anno}_annotation_result_by_qtl_{index}"))
        plt.show()
