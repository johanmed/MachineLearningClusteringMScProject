...
class Columns2Clustering(ModellingBirch):
    ...
    def visualize_plot(plot_birch, birch_clustering, X_train, size=500):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_BIRCH_clustering_SVM_prediction_result_by_qtl"))
        plt.show()
    ...
    def visualize_plot_annotation(X_valid, y_supervised_pred, type_anno):
        ...
        plt.savefig(os.path.join(out_dir, f"Project_PCA_BIRCH_clustering_SGD_{type_anno}_annotation_result_by_qtl"))
        plt.show()
...
print(f"Execution time for dbscan_clustering_svm_prediction_qtl.py is : {time_taken} seconds")
